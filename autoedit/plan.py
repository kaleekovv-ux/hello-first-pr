...
Этот модуль проверяет план и файлы проекта на уровне схемы:
обязательные поля, допустимые значения, существование файлов. Проверка
того, что anchor-фразы действительно встречаются в озвучке (и по
порядку) — отдельным шагом, в модуле align (нужно распознавание речи,
faster-whisper), она вызывается из cli.py/render.py уже после этой
схемной проверки.
"""
