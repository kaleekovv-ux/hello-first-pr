...
лог хода работы и кнопка «Открыть отчёт». Использует только стандартную
библиотеку Python (tkinter), поэтому не требует установки чего-либо
сверх самого AutoEdit — но сам tkinter должен быть в вашей установке
Python (в официальных сборках с python.org для Windows и macOS он есть
по умолчанию).
...
обычную командную строку (autoedit check / autoedit render).
"""
...
def _run_cli_in_thread(args: list[str], log_widget: "scrolledtext.ScrolledText", on_done) -> None:
    def worker() -> None:
        try:
            ...
            for line in process.stdout:
                log_widget.after(0, _append_log, log_widget, line)
            process.wait()
        finally:
...
def _append_log(log_widget: "scrolledtext.ScrolledText", text: str) -> None:
...
def run() -> None:
    ...
    def start(args: list[str]) -> None:
        ...
        _run_cli_in_thread([*args, project], log, lambda: set_buttons_enabled(True))

    def open_report() -> None:
        report_path = Path(project_var.get()) / "output" / "report.txt"
